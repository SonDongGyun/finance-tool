...
const COUNTS = { new: 3, removed: 1, increased: 5, decreased: 2 };
...
describe('StatusFilterBar', () => {
  ...
  it('invokes onChange with the clicked filter key', async () => {
    ...
    await user.click(screen.getByText('증가').closest('button'));
  ...
  it('renders compact size with smaller padding', () => {
    ...
    const defaultPadding = screen.getByText('전체').closest('button').style.padding;

    rerender(<StatusFilterBar value="all" onChange={() => {}} size="compact" />);
    const compactPadding = screen.getByText('전체').closest('button').style.padding;
